...
import { updateManagementCompany } from "../../actions";
...
export default function EditCompanyHeader({
  company,
  managersCount,
  artistsCount,
  knownArtistsCount,
  ...
  async function handleSave() {
    ...
    setSaving(true);
    setError(null);
    const result = await updateManagementCompany(company.id, {
      name: name.trim(),
    ...
    return (
      <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-[#001489]">{company.name}</h1>
            {company.website && (
              <a
    ...
    <div className="rounded-xl border border-[#001489]/30 bg-white p-6 shadow-sm">
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-semibold uppercase tracking-wider text-[#001489]">Edit Company</h2>
        ...
        <div>
          ...
          <input value={website} onChange={(e) => setWebsite(e.target.value)} placeholder="e.g. redlightmanagement.com" className={inputCls} />
        </div>
        <div>
